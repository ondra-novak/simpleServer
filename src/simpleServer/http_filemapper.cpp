...
void HttpFileMapper::operator ()(const HTTPRequest& req) {
	bool res = operator()(req,req.getPath());
	if (res == false) return req.sendErrorPage(404);

}

static const std::pair<StrViewA, StrViewA> mimeList[]={
		                {".323", "text/h323"},
		                {".3g2", "video/3gpp2"},
		                {".3gp", "video/3gpp"},
		                {".3gp2", "video/3gpp2"},
...
StrViewA HttpFileMapper::mapMime(StrViewA fullPathname) {
	auto sep = fullPathname.lastIndexOf(".");
	if (sep == fullPathname.npos) {
...
bool HttpFileMapper::operator ()(const HTTPRequest& req, const StrViewA& vpath) {
	...
	buffer.reserve(documentRoot.size()+1000);
	buffer.insert(buffer.end(),documentRoot.begin(),documentRoot.end());
